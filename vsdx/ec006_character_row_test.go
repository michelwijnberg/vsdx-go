...
// EC-006: ensureSectionCell used to create a partial local Row when an
...
func TestEC006_CharacterRowPreservesSiblingCells(t *testing.T) {
	...
	if len(expectedCells) < 2 {
		t.Skipf("master Character row has only %d cells; EC-006 needs >=2 to demonstrate", len(expectedCells))
	}
	...
	for name := range expectedCells {
		if !gotCells[name] {
			t.Errorf("local Character row missing cell %q — EC-006 regression (Visio would treat partial row as fully local and drop inherited %q)",
				name, name)
		}
	}
}
