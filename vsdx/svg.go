...
var visioArrowTypes = map[int]ArrowDef{
	...
	// Type 39: double-stack filled triangle. Tips forward (right side),
	// bases at refX=0 = line endpoint. End marker extends forward of the
	// line endpoint; begin marker uses auto-start-reverse to flip 180°
	// and point outward from line start. W=4 H=2 matches Visio's lend39
	// 2:1 aspect. Setback=W shortens the line on each end by the marker
	// width so the marker bases sit ON the original BeginX / EndX
	// positions and the total visual width equals Visio's (which uses
	// the same setback convention).
	39: {Path: "M0 0 L6 5 L0 10 z M4 0 L10 5 L4 10 z", W: 4.0, H: 2.0, RefX: 0, RefY: 5, Filled: true,  Setback: 4.0},
	40: {Path: "M0 0 L6 5 L0 10 M4 0 L10 5 L4 10", W: 4.0, H: 2.0, RefX: 0, RefY: 5, Filled: false, Setback: 4.0},
	...
	// Type 44: chevron + perpendicular bar. Visio's lend44 (mirrored to
	// tips-forward convention): apex at x=10 (forward), bases at x=3.33
	// (mid), bar at x=0 (AT refX = line endpoint). When rendered the bar
	// sits at the line endpoint perpendicular to it, then the chevron
	// extends outward. W=3 H=2 matches Visio's 1.5:1 aspect.
	44: {Path: "M3.33 0 L10 5 L3.33 10 M0 1.25 L0 8.75", W: 3.0, H: 2.0, RefX: 0, RefY: 5, Filled: false, Setback: 3.0},
