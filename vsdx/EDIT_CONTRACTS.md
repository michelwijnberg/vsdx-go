...
| ~~EC-007~~ | ~~`SetText` is half-destructief — leegt text-content van `<cp>`/`<pp>`/`<fld>` maar laat de elementen staan.~~ | ~~[shape.go:1326](shape.go)~~ | — | ✅ Gefixt met optie (a): SetText verwijdert nu alle child-elements van `<Text>` voor het de nieuwe string zet. cp/pp/fld referenties verwijzen naar tekstposities die na de overschrijving niet meer bestaan, dus blijven achterlaten produceert dode markers. De Character/Paragraph/Field SECTIES op de shape blijven onaangetast. Test: `TestSetTextContract_EC007_StripsFormatMarkerElements`. |
...
| ~~EC-009~~ | ~~Geen public `DisconnectShapes` / `RemoveConnect` als standalone API.~~ | ~~n.v.t.~~ | — | ✅ Twee API's toegevoegd: `(*Connect).Remove()` detacht één enkele binding; `(*Page).DisconnectShapes(connector, terminal)` verwijdert alle bindings die matchen, met nil-wildcard op elke kant. Connector-shape blijft staan met zijn geometrie — alleen de `<Connect>` elementen verdwijnen. Tests in [disconnect_shapes_contracts_test.go](disconnect_shapes_contracts_test.go). |
...
| ~~EC-013~~ | ~~`Page.GroupShapes` bbox-berekening gebruikt `Width()/2` ipv `LocX()` om stale V te omzeilen.~~ | ~~[foreign.go:172](foreign.go)~~ | — | ✅ Bbox-loop in `GroupShapes` gebruikt nu `(PinX - LocX, PinY - LocY, +Width, +Height)`. De oorspronkelijke stale-V zorg is sinds commits bdbf70d en 5b949ec ondervangen: SetWidth/SetHeight schalen LocPinX/Y nu mee, en Visio-gegenereerde files hebben V evaluated at save-time. Test omgezet naar `TestGroupShapesContract_BboxOffCenterPin` met tight-bbox verwachting. |
...
*Last updated*: 2026-05-29 (laatste pass: EC-007, EC-009 en EC-013 dichtgezet. EC-007 = SetText strip alle child elements; EC-009 = `(*Connect).Remove` + `(*Page).DisconnectShapes`; EC-013 = bbox in `GroupShapes` gebruikt nu LocX/LocY ipv Width/2. Alle EC-issues gesloten. DIVERGENCE_STATUS nul NEEDS_WORK items. Mutation-render corpus 8 recipes SSIM 1.000.).
